...
#
# Topology (all in Availability Zone 1, East US):
#
...
#     ├── App VM 0  (nginx + gunicorn)
#     └── App VM 1  (nginx + gunicorn)
#
...
# Outbound rule so App VMs can reach the internet (apt, git, pip)
...
# SSH NAT rules: LB:2201 → App VM 0:22 | LB:2202 → App VM 1:22
...
resource "azurerm_linux_virtual_machine" "db_replica" {
  ...
  # Replica must start after primary is created
