...
def fetch_all_customers(since=None):
    ...
    while True:
        ...
        if since:
            params["since"] = since
        resp = requests.get(f"{UPSTREAM_BASE_URL}/api/customers",
...
def upsert_customers(records: list, fetched_at: str):
    ...
    try:
        with conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                for r in records:
                    cur.execute("SELECT salary, company FROM customers WHERE id = %s", (r["id"],))
                    existing = cur.fetchone()
                    if existing:
                        cur.execute("""
                            UPDATE customers
                               SET first_name=%s, last_name=%s, email=%s,
                                   phone=%s, city=%s, fetched_at=%s
                             WHERE id=%s
                        """, (r.get("first_name",""), r.get("last_name",""),
                               r.get("email",""), r.get("phone",""),
                               r.get("city",""), fetched_at, r["id"]))
                    else:
                        cur.execute("""
                        ...
                        """, (r["id"], r.get("first_name",""), r.get("last_name",""),
                               r.get("email",""), r.get("phone",""),
                               r.get("city",""), fetched_at))
    finally:
        conn.close()
...
def edit(customer_id):
    ...
    if request.method == "POST":
        ...
        query("""
            UPDATE customers SET salary=%s, company=%s, updated_at=%s WHERE id=%s
